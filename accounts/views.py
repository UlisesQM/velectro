...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = 'Por favor activa la cuenta'
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(request, 'Tu email o contraseña son incorrectos!')
...
def logout(request):
    auth.logout(request)
    messages.success(request, "Ha cerrado sesión con éxito. ¡Vuelve pronto!")
    return redirect('accounts:login')
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        messages.error(request, "Link inválido para activar, inténtalo de nuevo!")
    ...
    if request.method == 'POST':
        ...
        if new_password == repeat_new_password:
            ...
            if success :
                ...
                messages.error(request, 'La contraseña anterior es incorrecta')
                return redirect('accounts:change_password')
        else:
...
def forget_password(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.error(request, '¡Este correo electrónico no existe!')
...
def reset_password(request):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, "¡Por favor ingrese su dirección de correo electrónico aquí primero! ")
            return redirect('accounts:forget_password')
