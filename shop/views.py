...
def shop2(request,subcategory_slug = None):
    subcategories = None
    # categories = None
    products = None
...
def shop(request, subcategory_slug=None,category_slug=None):
    # shop/subcategori
    # subcategories = None
    # if es un es otro
    # tercera opcion nombre
    categories = None
    subcategories = None
    products = None
...
def product_details(request,subcategory_slug,category_slug ,product_details_slug):
    try:
        single_product = Product.objects.get( subcategory__slug = subcategory_slug, category__slug = category_slug,slug=product_details_slug)
...
def review(request, product_id):
    ...
    if request.method == 'POST':
        try:
            ...
            form.save()
            messages.success(request, '¡Gracias, tu reseña actualizada!')
            return redirect(url)
            ...
            if form.is_valid():
                ...
                data.save()
                messages.success(request, '¡Gracias, tu reseña publicada!')
                return redirect(url)
