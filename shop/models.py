...
class Subcategory(models.Model):
    ...
    def get_subcategory_slug_url(self):
        return reverse('shop:subcategries', args=[self.slug])
...
class Category(models.Model):
    subcategory1 = models.ForeignKey(Subcategory, on_delete=models.CASCADE, default=True, null=False)
    name = models.CharField(max_length=100, unique=True)
    slug = models.SlugField(max_length=100, unique=True)
    ...
    def get_category_slug_url(self):
        return reverse('shop:categries', args=[self.subcategory1.slug,self.slug])

    # def get_subcategory_slug_url_child(self):
    #     return reverse('shop:categries', args=[self.subcategory1.slug,self.slug])
...
class Product(models.Model):
    subcategory = models.ForeignKey(Subcategory, on_delete=models.CASCADE, default=True, null=False)
    ...
    def get_prodcut_details_url(self):
        return reverse('shop:product_details', args=[self.subcategory.slug, self.category.slug,self.slug])
